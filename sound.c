	...
	if(REG_IF & INT_VBLANK) {
		if (soundA.isPlaying) {
			...
			if (soundA.vBlankCount > soundA.duration) {
				if (soundA.loops) {
					soundA.vBlankCount = 0; //not sure
					dma[1].cnt = 0;
			        DMANow(1, soundA.data, REG_FIFO_A, DMA_DESTINATION_FIXED | DMA_AT_REFRESH | DMA_REPEAT | DMA_32);
				} else {
					soundA.isPlaying = 0;
					dma[1].cnt = 0;//turn off the DMA channel the sound is using
					REG_TM0CNT = 0; //turn off the timer
				}
			}
		...
		if (soundB.isPlaying) {
			...
			if (soundB.vBlankCount > soundB.duration) {
				if (soundB.loops) {
					soundB.vBlankCount = 0; //not sure
					dma[2].cnt = 0;
			        DMANow(2, soundB.data, REG_FIFO_A, DMA_DESTINATION_FIXED | DMA_AT_REFRESH | DMA_REPEAT | DMA_32);
				} else {
					soundB.isPlaying = 0;
					dma[2].cnt = 0;//turn off the DMA channel the sound is using
					REG_TM1CNT = 0; //turn off the timer
				}
			}
